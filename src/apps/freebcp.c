...
static RETCODE format_column(BCPPARAMDATA* pdata, DBPROCESS* dbproc, BCPFORMAT file_format, int i)
{
	int li_numcols = dbproc->hostfileinfo->host_colcount;
...
static int file_process(BCPPARAMDATA * pdata, DBPROCESS* dbproc, DBINT dir)
{
	...
	case BCPFORMAT_NATIVE:
		for (i = 1; i <= li_numcols; ++i) {
			if (format_column(pdata, dbproc, file_format, i) == FAIL) {
