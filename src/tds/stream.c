...
static int tds_fileout_stream_write(TDSOUTSTREAM* stream, size_t n)
{
...
static
TDSRET tds_fileout_stream_putbuf(TDSFILEOUTSTREAM* s, void const* src, size_t n)
{
...
TDSRET tds_fileout_stream_put(TDSFILEOUTSTREAM* s, void const* src, size_t n)
{
	...
	if (s->buff_type == _IONBF)
		return fwrite(src, n, 1, s->fp) == 1 ? TDS_SUCCESS : TDS_FAIL;

	if (s->buff_type == _IOFBF)
