			  ...
			  int offset, unsigned char * rowbuffer, int start)
{
	...
	for (i = 0; i < bcpinfo->bindinfo->num_cols; i++) {
		...
		if (bcpcol->bcp_column_data->is_null) {
			tdsdump_log(TDS_DBG_ERROR, "tds_bcp_add_fixed_columns column %d is a null column\n", i + 1);
 ...
 */
static int
tds_bcp_add_variable_columns(TDSBCPINFO *bcpinfo, tds_bcp_get_col_data get_col_data, tds_bcp_null_error null_error,
