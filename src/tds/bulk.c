...
probe_sap_locking(TDSSOCKET *tds, TDSBCPINFO *bcpinfo)
{
	...
	if (rdot == NULL && tablename[0] == '#') {
		rc = tds_submit_queryf(tds, "select sysstat2 from tempdb..sysobjects where id = object_id('tempdb..%s')",
				       tablename);
	} else {
		rc = tds_submit_queryf(tds, "select sysstat2 from %.*ssysobjects where type='U' and name='%s'",
...
static bool
cbuf_match(const char *cbuf, size_t cbuf_size, size_t cpos, const char *match)
{
	if (cbuf_size == 1)
...
TDSRET
tds_file_stream_close(TDSFILESTREAM *stream)
{
