...
static TDS_INT
tds_convert_uint1(const TDS_TINYINT * src, int desttype, CONV_RESULT * cr)
{
	return tds_convert_int(*src, desttype, cr);
}
...
static inline void tds02dfast(char out[2], int v)
{
	out[0] = DIG2[v * 2];
	out[1] = DIG2[v * 2 + 1];
}

void tds_02dfast(char out[2], int v)
{
	tds02dfast(out, v);
}
...
size_t tds_u32toafast(char out[10], uint32_t v)
{
	...
	if (v < 10u) { *out = (char)('0' + v); return 1; }
	if (v < 100u) { tds02dfast(out, v); return 2; }

	pad = tds_u32toafast_right(buf, v);

	memcpy(out, buf + pad, 10 - pad);
	return 10 - pad;
}

size_t tds_i32toafast(char out[11], int32_t x)
{
	size_t sign_length = 0;
	if (x < 0) {
		*out++ = '-';
		sign_length = 1;
		if (TDS_LIKELY(x != (-2147483647 - 1)))
			x = -x;
	}
	return tds_u32toafast(out, x) + sign_length;
}
...
tds_convert_int(TDS_INT num, int desttype, CONV_RESULT * cr)
{
	...
	case TDS_CONVERT_CHAR:
	case CASE_ALL_CHAR:
		tmp_str[tds_i32toafast(tmp_str, num)] = 0;
...
tds_convert_to_binary(int srctype, const TDS_CHAR * src, TDS_UINT srclen, int desttype, CONV_RESULT * cr)
{
	...
	case CASE_ALL_CHAR:
		...
		return (TDS_INT) tds_char2hex(ib, cr->cb.len, src, srclen);
...
tds_convert(const TDSCONTEXT *tds_ctx, int srctype, const void *src, TDS_UINT srclen, int desttype, CONV_RESULT *cr)
{
	...
	case SYBBITN:
		...
		break;
	case SYBSINT1:
		length = tds_convert_int1((const int8_t *) src, desttype, cr);
		break;
	case SYBINT1:
	case SYBUINT1:
		length = tds_convert_uint1((const TDS_TINYINT *) src, desttype, cr);
		break;
	case SYBINT2:
	...
	for (pz = our_format; *pz; ) {
		...
		case 'z':
			...
			if (prec || pz <= our_format || pz[-1] != '.') {
				 ...
				 */
				size_t pad = tds_u32toafast_right(buf, dr->decimicrosecond & 0x7fffffff);
				if (pad > 3)
