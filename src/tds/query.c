...
tds7_write_param_def_from_params(TDSSOCKET * tds, const char* query, size_t query_len, TDSPARAMINFO * params)
{
	...
	for (i = 0; i < params->num_cols; ++i) {
		...
		if (ids[i].p) {
			...
			tds_put_string(tds, tds_dstr_cstr(name),
				       (int) tds_dstr_len(name));
		}
		tds_put_smallint(tds, ' ');
 ...
 */
size_t
tds_quote_id_rpc(TDSSOCKET * tds, char *buffer, const char *id, ssize_t idlen)
{
	size_t len;
 ...
 */
size_t
tds_quote_string(TDSSOCKET * tds TDS_UNUSED, char *buffer, const char *str,
		 ssize_t len)
{
