...
static FILE *open_test_file(char const *filename);
typedef enum
{
	PART_END,
	PART_SQL,
...
static FILE *
open_test_file(char const *filename)
{
	FILE* input_file = NULL;
	char in_file[256];

	// If no filename requsted, try blk_in.in in both the expected location and the current directory.
	if (filename)
