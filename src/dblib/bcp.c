...
_bcp_convert_out(DBPROCESS * dbproc, TDSCOLUMN *curcol, BCP_HOSTCOLINFO *hostcol, TDS_UCHAR **p_data, const char *bcpdatefmt)
{
	...
	if (is_datetime_type(srctype) && is_ascii_type(hostcol->datatype)) {
		...
		buflen = _bcp_strftime((char *) *p_data, 256, bcpdatefmt, &when, prec);
...
_bcp_exec_out(DBPROCESS * dbproc, DBINT * rows_copied)
{
	...
	while (tds_process_tokens(tds, &result_type, NULL, TDS_STOPAT_ROWFMT|TDS_RETURN_DONE|TDS_RETURN_ROW|TDS_RETURN_COMPUTE)
		...
		if (dbproc->hostfileinfo->firstrow > row_of_query)
			continue;
		if (dbproc->hostfileinfo->lastrow > 0 && row_of_query > dbproc->hostfileinfo->lastrow) {
...
_bcp_exec_in(DBPROCESS *dbproc, DBINT *rows_copied)
{
	...
	*rows_copied = 0;
	...
	if (!TDS_SUCCEED(tds_file_stream_init(&hoststream, fopen(dbproc->hostfileinfo->hostfile, "rb")))) {
	...
	for (;;) {
		...
		if (dbproc->hostfileinfo->lastrow > 0 && row_of_hostfile > dbproc->hostfileinfo->lastrow)
			break;
...
#define TDS_EOL "\n"
				...
				tds_file_stream_seek_set(&hoststream, row_end);
...
static int
_bcp_strftime(char *buf, size_t maxsize, const char *format, const TDSDATEREC *dr, int prec)
{
	char *pbuf = buf;

	if (format)
		return (int) tds_strftime(pbuf, maxsize, format, dr, prec);
	...
	*pbuf++ = ':';
	...
	if (prec > 0) {
		...
		*pbuf++ = '.';
		...
		tds_u32toa_fast_right(ibuf, dr->decimicrosecond & 0x7FFFFFFF);
		/* Skip first 3 digits here, as decimicrosecond should not exceed 9,999,999 */
		memcpy(pbuf, ibuf + 3, prec);
		pbuf += prec;
	}
	*pbuf = '\0';
