...
#define MAX_STACK (8*1024*1024)
	...
	if ( !(final_filename = try_read_login_info_base(&common_pwd,
							 filename))
	    &&	!(final_filename = try_read_login_info_base(&common_pwd,
							    "PWD")) ) {
	...
	printf("found %s.%s for %s in \"%s\"\n", common_pwd.SERVER,
	       common_pwd.DATABASE, common_pwd.USER, final_filename);
