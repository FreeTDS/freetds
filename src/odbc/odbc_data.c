...
/* For date/time types - ODBC specification says that SQL_DESC_LENGTH
 * should be the character string representation size.
 * https://learn.microsoft.com/en-us/sql/odbc/reference/syntax/sqlsetdescfield-function
 */
#define SET_INFO_DT(type, prefix, suffix) SET_INFO2(type, prefix, suffix, drec->sql_desc_display_size)

static int odbc_datetime_display_size(const TDSCOLUMN * col, const TDS_DBC * dbc)
{
	/* This logic should match that of odbc_tds2sql() code block which
	 * selects the format string and precision.
	 *
	 * "precision" here refers to the number of digits to be generated by the
	 * TDS-specific %z format specifier, which represents fractions of a second.
	 *
	 * This value SHOULD be set in col->column_prec. It is so, for MS types.
	 * However, the non-MS types sometimes do not actually set column_prec,
	 * so odbc_tds2sql() hardcodes precisions for those types.
	 */
	...
	switch (tds_get_conversion_type(col->on_server.column_type, col->on_server.column_size))
	{
	case SYBMSDATETIMEOFFSET:
		/* Offset formatting hardcodes a " +HH:MM" suffix */
		extra = 7;
		prec = col->column_prec;
		goto datetime;
	case SYBMSDATETIME2:
		prec = col->column_prec;
		goto datetime;
	...
	case SYBDATETIME4:
		prec = 0;
	datetime:
		fmt = tds_dstr_cstr(&dbc->datetime_fmt);
		break;
	case SYBMSTIME:
		prec = col->column_prec;
		goto time;
	...
	case SYBTIME:
		prec = 3;
	time:
		fmt = tds_dstr_cstr(&dbc->time_fmt);
		break;
	case SYBMSDATE:
	case SYBDATE:
		prec = 0;
		fmt = tds_dstr_cstr(&dbc->date_fmt);
		break;
	default:
		return 0;
	}

	return tds_strftime_maxsize(fmt, prec) + extra;
}

static void
data_msdatetime_set_type_info(TDSCOLUMN * col, struct _drecord *drec, const TDS_DBC *dbc)
{
	drec->sql_desc_display_size = odbc_datetime_display_size(col, dbc);
...
data_generic_set_type_info(TDSCOLUMN * col, struct _drecord *drec, const TDS_DBC* dbc)
{
	...
	int datetime_display_size = odbc_datetime_display_size(col, dbc);
	...
	case SYBDATETIME:
		drec->sql_desc_concise_type = SQL_TYPE_TIMESTAMP;
		drec->sql_desc_display_size = datetime_display_size;
		drec->sql_desc_octet_length = sizeof(TIMESTAMP_STRUCT);
		drec->sql_desc_precision = 3;
		drec->sql_desc_scale     = 3;
		drec->sql_desc_datetime_interval_code = SQL_CODE_TIMESTAMP;
		SET_INFO_DT("datetime", "'", "'");

	case SYBDATETIME4:
		/* MS ODBC driver gives 19 for display_size and 16 for length,
		 * the default display string has length 19. 
		 * This appears to be a MS bug that we don't need to replicate,
		 * the two length values should match. */
		drec->sql_desc_concise_type = SQL_TYPE_TIMESTAMP;
		drec->sql_desc_display_size = datetime_display_size;
		drec->sql_desc_octet_length = sizeof(TIMESTAMP_STRUCT);
		drec->sql_desc_datetime_interval_code = SQL_CODE_TIMESTAMP;
