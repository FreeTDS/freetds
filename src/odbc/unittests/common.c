...
odbc_read_login_info(void)
{
	...
	if (in) {
		fprintf(in,
			"[%s]\nDriver = %s\nDatabase = %s\nServername = %s\n",
			common_pwd.SERVER, common_pwd.DRIVER,
			common_pwd.DATABASE, common_pwd.SERVER);
		fclose(in);
		if (ini_override) {
	...
	if (ini_override && SQLGetConfigMode(&old_config_mode)) {
		...
		SQLWritePrivateProfileString(server, (LPCTSTR) T("Driver"),
					     (void *) T(common_pwd.DRIVER),
					     filename);
		SQLWritePrivateProfileString(server, (LPCTSTR) T("Database"),
					     (LPCTSTR) T(common_pwd.DATABASE),
					     filename);
		SQLWritePrivateProfileString(server,
					     (LPCTSTR) T("Servername"),
					     (LPCTSTR) T(common_pwd.SERVER),
					     filename);
...
odbc_connect(void)
{
	...
	printf("connection parameters:\nserver:   '%s'\nuser:     '%s'\npassword: '%s'\ndatabase: '%s'\n",
	       ...
	       "????" /* common_pwd.PASSWORD */, common_pwd.DATABASE);
	...
	if (!odbc_conn_additional_params) {
		CHKConnect(T(common_pwd.SERVER), SQL_NTS, T(common_pwd.USER),
			   SQL_NTS, T(common_pwd.PASSWORD), SQL_NTS, "SI");
	} else {
		char *params;
		SQLSMALLINT len;
