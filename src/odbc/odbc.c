...
static char software_version[] = "$Id: odbc.c,v 1.288.2.7 2004-08-09 09:02:59 freddy77 Exp $";
...
odbc_upper_column_names(TDS_STMT * stmt)
{
	...
	for (icol = ird->header.sql_desc_count; --icol >= 0;) {
		...
		/* upper case */
		tds_ascii_strupr(drec->sql_desc_label);
		tds_ascii_strupr(drec->sql_desc_name);
	}
}
